                ...
                if (buffer[i] == ',' || buffer[i] == '\n') {
                    ...
                    for (int k = 0; k < lengths[current]; k++) {
                        if (boxes[current][k] != '\0') {
                            current_text_length++;
                            current_text = realloc(current_text, current_text_length);
                            current_text[current_text_length-1] = boxes[current][k];
                        }
                        else {
                            if (text_length == current_text_length && memcmp(&text[0], &current_text[0], current_text_length) == 0) {
                    ...
                    if (!found) {
                        ...
                        memcpy(boxes[current] + lengths[current] - (text_length + 3), text, text_length);
                    ...
                    for (int k = 0; k < lengths[current]; k++) {
                        if (boxes[current][k] != '\0') {
                            ...
                            if (text_length == current_text_length && memcmp(&text[0], &current_text[0], current_text_length) == 0) {
                                lengths[current] -= text_length + 3;
                                for (int y = k; y < lengths[current] + text_length; y++) {
                                    boxes[current][y - text_length] = boxes[current][y + 3];
                                }
                                boxes[current] = realloc(boxes[current], lengths[current]);
                                break;
                            }
