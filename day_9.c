...
int part_one() {
	...
	for (int y = 0; y < HISTORIES; y++) {
		...
		while (1) {
			...
			number_table =
				realloc(number_table, (NUMBERS + 1) * iteration * sizeof(int));
			...
			for (int i = 0; i < NUMBERS - iteration + 1; i++) {
				number_table[(NUMBERS + 1) * (iteration - 1) + i] =
					number_table[(NUMBERS + 1) * (iteration - 2) + i + 1] -
					number_table[(NUMBERS + 1) * (iteration - 2) + i];
			}
		}
		...
		for (int i = iteration - 1; i > 0; i--) {
			number_table[(NUMBERS + 1) * (i - 1) + NUMBERS + 1 - i] =
				number_table[(NUMBERS + 1) * (i - 1) + NUMBERS - i] +
				number_table[(NUMBERS + 1) * (i - 1) + NUMBERS * 2 + 1 - i];
		}
