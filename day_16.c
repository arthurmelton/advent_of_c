...
int part_one() {
    ...
    int gone_through[HEIGHT * (WIDTH + 1)] = {[0 ... HEIGHT * (WIDTH + 1) - 1] = 0};
    ...
    while(splitters_length << 1 != splitters_done) {
        ...
        if (position < 0 || position > sizeof(buffer) || buffer[position] == '\n') {
            splitters_done++;
            position = -1;
            continue;
        }
        ...
        else if ((buffer[position] == '-' && (direction == WIDTH + 1 || direction == -(WIDTH + 1))) || (buffer[position] == '|' && (direction == 1 || direction == -1))) {
            splitters_done++;
            int new = 1;
